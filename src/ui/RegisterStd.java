package ui;

import service.RegisterManager;
import service.Student;
import service.User;
import util.ImageUtils;
import util.RoundedLineBorder;
import util.RoundedPanel;

import javax.swing.*;
import javax.swing.border.Border;
import javax.swing.border.EmptyBorder;
import java.awt.*;

public class RegisterStd extends JFrame {

    // ตัวแปรสำหรับเก็บค่าที่ส่งมาจากหน้าแรก
    private String tempName;
    private String tempEmail;
    private String tempPassword;

    // --- แก้ไข Constructor ให้รับค่า ---
    public RegisterStd(String name, String email, String password) {
        this.tempName = name;
        this.tempEmail = email;
        this.tempPassword = password;

        setTitle("Sign Up - Myapp");
        setDefaultCloseOperation(EXIT_ON_CLOSE);
        setSize(900, 600);
        setMinimumSize(new Dimension(780, 540));
        setLocationRelativeTo(null);

        // ... (Code Setup UI เหมือนเดิมเป๊ะ ละไว้เพื่อความสั้น) ...
        // ... ก๊อปปี้ UI setup มาใส่ตรงนี้ ...
        // ... (หรือใช้ของเดิมที่คุณมี แต่เปลี่ยนแค่ส่วน Constructor header และ ปุ่มข้างล่างนี้) ...

        JPanel root = new JPanel(new BorderLayout());
        root.setBackground(Color.WHITE);
        setContentPane(root);

        // --- Setup GridBagLayout เหมือนเดิม ---
        JPanel main = new JPanel(new GridBagLayout());
        main.setBackground(new Color(248, 250, 252));
        main.setBorder(new EmptyBorder(24, 24, 24, 24));
        GridBagConstraints gc = new GridBagConstraints();
        gc.insets = new Insets(12, 12, 12, 12);
        gc.fill = GridBagConstraints.BOTH;

        // LEFT PANEL
        JPanel left = new JPanel(new BorderLayout());
        left.setBackground(Color.WHITE);
        left.setBorder(new EmptyBorder(12, 12, 12, 12));
        JLabel sut = new JLabel(ImageUtils.loadScaled("assets/hero.png", 360, 280));
        sut.setHorizontalAlignment(SwingConstants.CENTER);
        left.add(sut, BorderLayout.CENTER);
        gc.gridx = 0; gc.gridy = 0; gc.weightx = 1.2; gc.weighty = 1;
        main.add(left, gc);

        // RIGHT PANEL
        JPanel right = new RoundedPanel(18, Color.WHITE, new Color(229, 231, 235));
        right.setLayout(new GridBagLayout());
        right.setBorder(new EmptyBorder(24, 24, 24, 24));
        GridBagConstraints g2 = new GridBagConstraints();
        g2.gridx = 0; g2.fill = GridBagConstraints.HORIZONTAL;
        g2.insets = new Insets(6, 12, 6, 12); g2.weightx = 1.0;

        // ... ใส่ Header, Subtitle ...
        JLabel h1 = new JLabel("SIGN UP");
        h1.setFont(h1.getFont().deriveFont(Font.BOLD, 26f));
        h1.setHorizontalAlignment(SwingConstants.CENTER);
        g2.gridy = 0; right.add(h1, g2);

        JLabel subtitle = new JLabel("Create your STUDENT ID");
        subtitle.setHorizontalAlignment(SwingConstants.CENTER);
        g2.gridy = 1; right.add(subtitle, g2);

        // Student ID Input
        JLabel stdlist = new JLabel("Student ID");
        g2.gridy = 2; right.add(stdlist, g2);

        JTextField txtStd = new JTextField(20);
        txtStd.setBorder(compoundRoundedBorder(14, new Color(203, 213, 225), 2, new Insets(10, 12, 10, 12)));
        g2.gridy = 3; right.add(txtStd, g2);

        // Button
        JButton btnRegister = new JButton("Create an account");
        btnRegister.setFocusPainted(false);
        btnRegister.setForeground(Color.WHITE);
        btnRegister.setBackground(new Color(251, 146, 60));
        btnRegister.setPreferredSize(new Dimension(0, 44));

        // --- แก้ไข LOGIC ปุ่มกด ---
        btnRegister.addActionListener(e -> {
            String stdId = txtStd.getText().trim();

            if (stdId.isEmpty()) {
                JOptionPane.showMessageDialog(this, "Please enter Student ID", "Error", JOptionPane.ERROR_MESSAGE);
                return;
            }


            // สร้าง User แบบ Student (ID=0 เพราะ DB auto increment, Status="Student")
            User newUser = new Student(0, tempName, tempEmail, tempPassword, "Student" ,stdId);

            // เรียก Service เพื่อบันทึก
            boolean success = RegisterManager.register(newUser);

            if (success) {
                JOptionPane.showMessageDialog(this, "Account created successfully!", "Success", JOptionPane.INFORMATION_MESSAGE);
                dispose();
                SwingUtilities.invokeLater(() -> new Login().setVisible(true));
            } else {
                JOptionPane.showMessageDialog(this, "Registration Failed (Email might be taken)", "Error", JOptionPane.ERROR_MESSAGE);
            }
        });

        g2.gridy = 12; g2.insets = new Insets(12, 12, 6, 12);
        right.add(btnRegister, g2);

        // ... (Layout ปิดท้าย) ...
        g2.gridy = 14; g2.weighty = 1; right.add(Box.createVerticalStrut(1), g2);
        gc.gridx = 1; gc.gridy = 0; gc.weightx = 1.0; gc.weighty = 1;
        main.add(right, gc);
        root.add(main, BorderLayout.CENTER);
        getRootPane().setDefaultButton(btnRegister);
    }

    private static Border compoundRoundedBorder(int radius, Color lineColor, int thickness, Insets innerPadding) {
        return BorderFactory.createCompoundBorder(
                new RoundedLineBorder(lineColor, thickness, radius),
                new EmptyBorder(innerPadding));
    }

    // Main สำหรับเทส (ต้องแก้ให้ส่งค่าว่างไปก่อน ไม่งั้น Error)
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> new RegisterStd("Test", "test@email.com", "1234").setVisible(true));
    }
}